...
vec3 ImportanceSampleGGX(vec2 Xi, vec3 N, float uRoughness)
{
...
void main()
{
    vec3 N = normalize(vPositionInModel);
    vec3 R = N;
    vec3 V = R;
    ...
    for (uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        if (NdotL > 0.0)
        {
            // reduce dotted artifact by not directly sampling the environment map,
            // but sampling a mip level of the environment map based on the integral's PDF and the roughness: 
