...
{
	vec2 p = vTexCoord.xy / uResolution.xy - 0.5;
	vec2 q = vTexCoord.xy / uResolution.xy;
	p.x *= uResolution.x / uResolution.y;
	vec2 mo = iMouse.xy / uResolution.xy - .5;
...
void main()
{
	...
	ro.y -= height(ro.zx) + 0.05;
	// mo.x += smoothstep(0.6, 1., sin(time*.6)*0.5 + 0.5) - 1.5;
	vec3 eyedir = normalize(vec3(0.0), - 0.2 + sin(time*0.45*1.57)*0.1, 0.0));
	vec3 rightdir = normalize(vec3(cos(1.5708), 0., sin(1.5708)));
	vec3 updir = normalize(cross(rightdir, eyedir));
	vec3 rd = normalize((p.x*rightdir + p.y*updir)*1. + eyedir);
	*/
