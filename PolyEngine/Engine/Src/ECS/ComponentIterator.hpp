...
namespace Poly
{
							...
							public std::iterator<std::forward_iterator_tag, std::tuple<typename std::add_pointer<PrimaryComponent>::type, typename std::add_pointer<SecondaryComponents>::type...>>	
	{
		public:
			...
			bool operator==(const  ComponentIterator& rhs) const { return Iter.get()->get() == GetIteratorHelper()->get(); } //they never  trigger underlying overload, pointer comparison ongoing
			bool operator!=(const  ComponentIterator& rhs) const { return !(Iter.get()->get() == GetIteratorHelper()->get()); }
