...
RTTI_DEFINE_TYPE(pe::engine::res::MeshResource)
...
MeshResource::SubMesh::SubMesh(const core::storage::String& path, aiMesh* mesh, aiMaterial* material)
{
	...
	MeshData.EmissiveMap			= LoadTexture(material, path, (unsigned int)aiTextureType_EMISSIVE,		api::rendering::eTextureUsageType::EMISSIVE);
	MeshData.AlbedoMap				= LoadTexture(material, path, (unsigned int)aiTextureType_DIFFUSE,		api::rendering::eTextureUsageType::ALBEDO);
	MeshData.MetallicMap			= LoadTexture(material, path, (unsigned int)aiTextureType_SPECULAR,		api::rendering::eTextureUsageType::METALLIC);
	MeshData.RoughnessMap			= LoadTexture(material, path, (unsigned int)aiTextureType_SHININESS,	api::rendering::eTextureUsageType::ROUGHNESS);
	MeshData.NormalMap				= LoadTexture(material, path, (unsigned int)aiTextureType_NORMALS,		api::rendering::eTextureUsageType::NORMAL);
	MeshData.AmbientOcclusionMap	= LoadTexture(material, path, (unsigned int)aiTextureType_AMBIENT,		api::rendering::eTextureUsageType::AMBIENT_OCCLUSION);
}

void MeshResource::LoadBones(aiNode* node)
{
...
TextureResource* MeshResource::SubMesh::LoadTexture(const aiMaterial* material, const core::storage::String& path, const unsigned int aiType, const api::rendering::eTextureUsageType textureType)
{
	...
	if (material->GetTexture(type, 0, &texturePath) == AI_SUCCESS)
	{
		...
		texture = ResourceManager<TextureResource>::Load(textPath, config::eResourceSource::NONE, textureType);
		if (!texture) {
...
MeshResource::Animation::Animation(aiAnimation * anim)
{
...
MeshResource::Animation::ChannelLerpData MeshResource::Animation::GetLerpData(::pe::core::storage::String channelName, float time) const
{
