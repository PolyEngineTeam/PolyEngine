...
TEST_CASE("Angle constructors", "[Angle]") {
  ...
  // radians constructor
  Angle a2 = Angle::FromRadians(M_PI);
  REQUIRE(Cmpf(a2.AsRadians(), M_PI) == true);
  ...
  REQUIRE(Cmpf(a3.AsRadians(), M_PI) == true);
  REQUIRE(Cmpf(a4.AsDegrees(), 180.0f) == true);
  REQUIRE(Cmpf(a4.AsRadians(), M_PI) == true);
  
  Angle a5 = 3.14159265358979323846264338327950288_rad;
  REQUIRE(Cmpf(a5.AsDegrees(), 180.0f) == true);
  REQUIRE(Cmpf(a5.AsRadians(), M_PI) == true);
}

TEST_CASE("Comparison operatos", "[Angle]") {
