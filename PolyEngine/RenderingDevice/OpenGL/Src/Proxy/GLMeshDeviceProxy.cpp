...
void GLMeshDeviceProxy::SetContent(const Mesh& mesh)
{
	...
	if (mesh.HasVertices())
	{
		...
		glBufferData(GL_ARRAY_BUFFER, mesh.GetPositions().size() * sizeof(Vector3f), mesh.GetPositions().data(), GL_STATIC_DRAW);
	...
	if (mesh.HasNormals())
	{
		...
		glBufferData(GL_ARRAY_BUFFER, mesh.GetNormals().size() * sizeof(Vector3f), mesh.GetNormals().data(), GL_STATIC_DRAW);
	...
	if(mesh.HasTangents())
	{
		...
		glBufferData(GL_ARRAY_BUFFER, mesh.GetTangents().size() * sizeof(Vector3f), mesh.GetTangents().data(), GL_STATIC_DRAW);
	...
	if(mesh.HasBitangents())
	{
		...
		glBufferData(GL_ARRAY_BUFFER, mesh.GetBitangents().size() * sizeof(Vector3f), mesh.GetBitangents().data(), GL_STATIC_DRAW);
	...
	if (mesh.HasBoneIds())
	{
		...
		// glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh.GetBoneIds().size() * sizeof(VectorT<u8, 4>), mesh.GetBoneIds().data(), GL_STATIC_DRAW);
