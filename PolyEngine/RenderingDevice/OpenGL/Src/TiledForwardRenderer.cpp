...
void TiledForwardRenderer::RenderLit(World* world, const AARect& rect, const CameraComponent* cameraCmp)
{
	gConsole.LogInfo("TiledForwardRenderer::RenderLit");

	const EntityTransform cameraTrans = cameraCmp->GetOwner()->GetTransform();
	...
	hdrShader.SetUniform("near", cameraCmp->GetClippingPlaneNear());
	hdrShader.SetUniform("far", cameraCmp->GetClippingPlaneFar());
...
void TiledForwardRenderer::DrawMeshes(World* world, GLShaderProgram& shader, const Matrix& mvp)
{
	...
	for (auto componentsTuple : world->IterateComponents<MeshRenderingComponent>())
	{
		...
		const EntityTransform meshTrans = meshCmp->GetOwner()->GetTransform();
		const Matrix& ModelTransform = meshTrans.GetGlobalTransformationMatrix();
...
void TiledForwardRenderer::UpdateLights(World* world) const
{
	...
	unsigned int i = 0;
	for (auto componentsTuple : world->IterateComponents<PointLightComponent>())
	{
		const PointLightComponent* pointLightCmp = std::get<PointLightComponent*>(componentsTuple);
		const EntityTransform lightTrans = pointLightCmp->GetOwner()->GetTransform();

		PointLight &light = pointLights[i];
		light.position = lightTrans.GetGlobalTranslation();
