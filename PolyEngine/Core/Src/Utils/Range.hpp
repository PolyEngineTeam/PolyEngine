...
namespace Poly {
    ...
    template <typename IT> typename Poly::Range<IT>::Iterator_t begin(const Poly::Range<IT>& rhs) noexcept { return rhs.Begin(); }
	template <typename IT> typename Poly::Range<IT>::Iterator_t end(const Poly::Range<IT>& rhs) noexcept { return rhs.End(); }
