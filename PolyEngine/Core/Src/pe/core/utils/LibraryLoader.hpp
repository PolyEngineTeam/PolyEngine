...
#error "Unsupported platform :("
	...
	LibraryFunctionHandle<Function> LoadFunctionFromSharedLibrary(const char* libraryName, const char* functionSymbol)
	{
		...
		if (!libHandle)
		{
			...
			::pe::core::utils::gConsole.LogError("Shared library [{}] load failed. Error code: {}", libraryName, err);
		...
		{
			DWORD err = GetLastError();
			::pe::core::utils::gConsole.LogError("Function [{}] from shared library [{}] load failed. Error code: {}", functionSymbol, libraryName, err);
			return LibraryFunctionHandle<Function>{ nullptr, nullptr };
		}
#elif defined(__linux__) || defined(__APPLE__)
		...
		{ //we could simply check if the handle is null, but using dlerror() doubles as clearing error flags
			::pe::core::utils::gConsole.LogError("Shared library [{}] load failed. Error: {}", libraryName, err);
			return LibraryFunctionHandle<Function>{ nullptr, nullptr };
		}
		...
		{ //symbols can be legally null, so we need to check for errors instead
			::pe::core::utils::gConsole.LogError("Function [{}] from shared library [{}] load failed. Error: {}", functionSymbol, libraryName, err);
			dlclose(libHandle);
			return LibraryFunctionHandle<Function>{ nullptr, nullptr };
		}
#else
#error "Unsupported platform :("
		...
		::pe::core::utils::gConsole.LogError("Function [{}] from shared library [{}] loaded succesfully!", functionSymbol, libraryName);
