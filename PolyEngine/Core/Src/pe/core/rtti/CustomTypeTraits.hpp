...
namespace Poly
{
	namespace Trait
	{
		...
		template <typename T, typename E> struct IsEnumArray<::pe::core::utils::EnumArray<T, E>> : public std::true_type {};

		template <typename> struct EnumArrayType {};
		template <typename T, typename E> struct EnumArrayType<::pe::core::utils::EnumArray<T, E>> { using enumType = E; using valueType = T; };

		// Is enum flags
		template <typename> struct IsEnumFlags : public std::false_type {};
		template <typename E> struct IsEnumFlags<::pe::core::utils::EnumFlags<E>> : public std::true_type {};

		template <typename> struct EnumFlagsType {};
		template <typename E> struct EnumFlagsType<::pe::core::utils::EnumFlags<E>> { using type = E; };

		// Is iterable pool allocator
		template <typename> struct IsIterablePoolAllocator : public std::false_type {};
		template <typename E> struct IsIterablePoolAllocator<::pe::core::memory::IterablePoolAllocator<E>> : public std::true_type {};

		template <typename> struct IterablePoolAllocatorType {};
		template <typename E> struct IterablePoolAllocatorType<::pe::core::memory::IterablePoolAllocator<E>> { using type = E; };
