...
void SGJGame::Init()
{
	...
	Engine->RegisterComponent<SGJ::LevelComponent>((int)eGameComponents::LEVELCOMPONENT);
	...
	UniqueID level = DeferredTaskSystem::SpawnEntityImmediate(Engine->GetWorld());
	DeferredTaskSystem::AddComponentImmediate<SGJ::LevelComponent>(Engine->GetWorld(), level, "Levels/sampleLevel.csv");
};
